...
public class JB_Statistics : MonoBehaviour{
    ...
    public JB_Statistics(float toughness, float strength, float dexterity, float knowledge, float reflex, float luck,float interactRange, float attackRange, int maxHp){
    ...
    public void DealDmg(){ //used for debug
        if (Input.GetKeyDown(KeyCode.E)){
            dealingDmg = true;
            currentHP -= 10;
            FindObjectOfType<JB_UIDamageNr>().Display();
